...
addEventListener('scroll',()=>{plates.forEach(im=>{const r=im.parentElement.getBoundingClientRect();const p=(r.top+r.height/2-innerHeight/2)/innerHeight;im.style.transform=`translateY(${p*-40}px) scale(1.08)`})},{passive:true});
...
async function load(){const [prods,man]=await Promise.all([fetch('../products.json').then(r=>r.json()),fetch('manifest.json').then(r=>r.json()).catch(()=>({}))]);return {prods,man}}
function card(p,man){const has=(man[p.slug]||[]).includes('primary');const price=PRICE[p.slug]||49;
  return `<a class="card rv ${has?'':'pending'}" href="product.html?s=${p.slug}"><div class="im">${has?`<img src="../img/products/web/${p.slug}-primary.jpg" alt="${p.name}">`:`<span>RENDERING</span>`}</div><div class="meta"><div class="name">${p.name}</div><div class="dose">${p.spray?'nasal spray':'lyophilised vial'} · 99% purity</div><div class="row"><span>$${price}.00</span><span class="buy">Add →</span></div></div></a>`}
...
if(pdp){load().then(({prods,man})=>{const s=new URLSearchParams(location.search).get('s')||'bpc-157';const p=prods.find(x=>x.slug===s)||prods[0];const kinds=['primary','frost','pigment','water','machine'].filter(k=>(man[p.slug]||[]).includes(k));const srcs=kinds.map(k=>`../img/products/web/${p.slug}-${k}.jpg`);
  ...
  srcs.forEach((src,i)=>{const im=new Image();im.src=src;im.alt=p.name;if(i===0)im.classList.add('on');car.prepend(im);const b=document.createElement('button');b.innerHTML=`<img src="${src}" alt="">`;if(i===0)b.classList.add('on');b.onclick=()=>go(i);th.appendChild(b)});
