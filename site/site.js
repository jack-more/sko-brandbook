...
if(pdp){load().then(({prods,man})=>{const s=new URLSearchParams(location.search).get('s')||'bpc-157';const p=prods.find(x=>x.slug===s)||prods[0];const kinds=['primary','white','frost','pigment','water'].filter(k=>(man[p.slug]||[]).includes(k));const srcs=kinds.map(k=>`../img/products/web/${p.slug}-${k}.jpg`);
