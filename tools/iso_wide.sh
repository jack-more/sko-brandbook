...
python3 -c "import json;[print(p['slug'],'|',p['label'],'|',int(p['spray'])) for p in json.load(open('products.json'))]" | while IFS='|' read slug label spray; do
  ...
  P="Wide product photograph outdoors at dusk, $GL. Composition: the left third and the right third of the frame are calm empty snow and sky with nothing in them; all of the subject sits in the middle third. In the middle third: $body stands upright in the snow toward the left, and the molecular structure drawn in the second reference, its exact skeleton, built as a chrome machine, every bond a knurled chrome strut, every atom a hex-nut joint, stands in the snow just to its right, rising to about three times its height with its lowest joints sunk into the snow. The product is exactly the first reference with its label reproduced letter for letter, the name printed exactly '$label', the dose pill exactly as printed, 99% Purity and Research Use Only, a deep navy flip-off cap over a silver crimp; every letter fully visible. No letters on the structure. No frames, borders or panels drawn in the picture. No text anywhere except the label. Only one product."
  (
