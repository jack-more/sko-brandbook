...
one(){ local src=$1 name=$(basename ${1%.*})
  ...
  read w h < <(python3 -c "from PIL import Image;im=Image.open('$src');print(im.size[0]*2,im.size[1]*2)")
  for i in 1 2 3; do higgsfield generate create topaz_image --image "$src" --output_width $w --output_height $h --variant CGI --wait > img/products/logs/hd-$name.log 2>&1 < /dev/null
