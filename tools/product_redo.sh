...
while read slug kind; do
  ...
  LABEL="the product is exactly the first reference, $body with its label reproduced letter for letter, and with exactly the first reference's closure: a deep navy blue flip-off cap, never light blue, never cerulean, over a plain silver crimp band: the holographic shield beside the white SKO COMPOUNDS lockup, then the product name printed exactly as '$label' and nothing longer, do not expand or complete the name, then the dose in a small hexagonal holographic pill exactly as printed on the reference, then 99% Purity and Research Use Only, holographic bands; every letter fully visible. No text anywhere except the label. Only one product."
  case $kind in
    ...
    white) ref=img/ref2/scene/white-ref.jpg; a=1:1; scene="Reproduce the second reference exactly: the same seamless pure white studio ground and backdrop, the same soft light, the same soft grey contact shadow, and the same framing, the vial the same size and in the same position as in the second reference";;
