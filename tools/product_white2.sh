...
python3 -c "import json;[print(p['slug'],'|',p['label'],'|',int(p['spray'])) for p in json.load(open('products.json'))]" | while IFS='|' read slug label spray; do
  ...
  (
    for i in 1 2 3; do higgsfield generate create nano_banana_2 --image img/ref2/cat/$slug.png --image img/ref2/badge/$slug.jpg --aspect_ratio 1:1 --wait --prompt "$P" > img/products/logs/$slug-white2.log 2>&1 < /dev/null; url=$(grep -oE 'https://[^ "]+\.png' img/products/logs/$slug-white2.log | head -1); echo "$slug white2 $url"; [ -n "$url" ] && curl -sL "$url" -o "$O/$slug-white2.png" && break; sleep 12; done
  ) &
