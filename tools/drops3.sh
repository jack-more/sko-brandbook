...
im=Image.open('img/products/web/$s-wide.jpg').convert('RGB'); im.thumbnail((1280,1280)); im.save('img/vid/_in/$s-end.jpg',quality=92)"
  for i in 1 2 3; do
    higgsfield generate create wan3_0 --start-image "img/vid/_in/$s-end.jpg" --end-image img/vid/_in/scene.jpg \
      --duration 2 --resolution 1080p --aspect_ratio 16:9 --generate-audio false --wait \
  ...
  ffmpeg -v error -i "img/vid/_in/$s-rev.mp4" -vf reverse -an -c:v libx264 -profile:v main -pix_fmt yuv420p -crf 22 -preset slow -movflags +faststart -vf "reverse,scale=1920:-2" "img/vid/$s.mp4" -y
  ffmpeg -v error -i "img/vid/$s.mp4" -an -c:v libvpx-vp9 -crf 30 -b:v 0 -row-mt 1 -speed 3 -pix_fmt yuv420p "img/vid/$s.webm" -y
  echo "$s ok"
}
