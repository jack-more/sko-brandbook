...
python3 -c "import json;[print(p['slug'],'|',p['label'],'|',int(p['spray'])) for p in json.load(open('products.json'))]" | while IFS='|' read slug label spray; do
  ...
  P="Reproduce the second reference exactly as a square photograph: the same snow, the same sky gradient, the same light and glints, the same product and the same chrome structure with the same shape, now composed to fit a square frame: $body stands upright in the snow at the lower left and the whole structure stands to its right, every atom of it inside the frame with clear margin on all sides, the structure scaled down as needed. The product is exactly the first reference with its label reproduced letter for letter, the name printed exactly '$label', the dose pill exactly as printed, 99% Purity and Research Use Only; every letter fully visible. Flawless mirror chrome, no hex nuts, no bolts. No frames or bands drawn in the picture. No text anywhere except the label. Only one product."
  (
