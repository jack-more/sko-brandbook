...
python3 -c "import json;[print(p['slug'],'|',p['label'],'|',int(p['spray'])) for p in json.load(open('products.json'))]" | while IFS='|' read slug label spray; do
  ...
  (
    for i in 1 2 3; do higgsfield generate create nano_banana_2 --resolution 4k --image img/ref2/cat/$slug.png --image img/ref2/wide/$slug.jpg --aspect_ratio 1:1 --wait --prompt "$P" > img/products/logs/$slug-square.log 2>&1 < /dev/null; url=$(grep -oE 'https://[^ "]+\.png' img/products/logs/$slug-square.log | head -1); echo "$slug square $url"; [ -n "$url" ] && curl -sL "$url" -o "$O/$slug-square.png" && break; sleep 12; done
  ) &
